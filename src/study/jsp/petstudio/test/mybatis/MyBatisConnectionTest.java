package study.jsp.petstudio.test.mybatis;
...
public class MyBatisConnectionTest {
	public static void main(String[] args){
		...
		/** 1차 작업 후 이클래스에서 에러가 발생한 경우 다음을 확인*/
		// 1) log4j2.xml파일에서 로그가 저장될 경로 설정이 정확한지 여부
		// 2) config.xm파일에서 데이터베이스 접속 정보 설정 확인
		// 3) MyBatisConnectionFactory에서 config.xml의 경로 설정 확인
		
		/** Mapper 파일의 경로를 config.xml에 추가할 때 마다 이 클래스로 테스트한다. */
